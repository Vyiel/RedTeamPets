...
abuseIpdbApi = "1a217419b3ab9b4fe153487826113964551f663190ca1e00fa024f345494f3f1b386527adf26d917"
abuseIpdbCat = {
    1: "DNS Compromise",
    2: "DNS Poisoning",
    3: "Fraud Orders",
...
def writeToDB(ip, score, domain, country, categories, is_blocked):
    ...
    qry = "INSERT IGNORE INTO ipaddresses (ip, score, domain, country, categories, is_blocked) VALUES (%s, %s, %s, %s, %s, %s)"
    values = (ip, score, domain, country, categories, is_blocked)
...
def writeIOCs(path):
    with open(path) as csvfile:
        ...
        if csVerify is True:
            cprint(f"Starting IOC block activity! ")
            for row in reader:
                firstRow = row
                if firstRow[0].lower().strip() == "ioc" and firstRow[1].lower().strip() == 'about':
                    pass
